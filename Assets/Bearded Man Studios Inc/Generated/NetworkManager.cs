...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] DreamObjectBaseNetworkObject = null;
		public GameObject[] DreamSpawnerNetworkObject = null;
		public GameObject[] ExampleProximityPlayerNetworkObject = null;
		public GameObject[] NetworkCameraNetworkObject = null;
		public GameObject[] NightmareObjectNetworkObject = null;
		public GameObject[] PlayerNetworkObject = null;
		public GameObject[] ScoreTallyNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is DreamObjectBaseNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (DreamObjectBaseNetworkObject.Length > 0 && DreamObjectBaseNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(DreamObjectBaseNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<DreamObjectBaseBehavior>();
						}
					}
			...
			else if (obj is DreamSpawnerNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (DreamSpawnerNetworkObject.Length > 0 && DreamSpawnerNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(DreamSpawnerNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<DreamSpawnerBehavior>();
						}
					}
			...
			else if (obj is NightmareObjectNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (NightmareObjectNetworkObject.Length > 0 && NightmareObjectNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(NightmareObjectNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<NightmareObjectBehavior>();
						}
					}
			...
			else if (obj is ScoreTallyNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (ScoreTallyNetworkObject.Length > 0 && ScoreTallyNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(ScoreTallyNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<ScoreTallyBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateDreamObjectBase instead, its shorter and easier to type out ;)")]
		public DreamObjectBaseBehavior InstantiateDreamObjectBaseNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(DreamObjectBaseNetworkObject[index]);
			var netBehavior = go.GetComponent<DreamObjectBaseBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<DreamObjectBaseBehavior>().networkObject = (DreamObjectBaseNetworkObject)obj;

			FinalizeInitialization(go, netBehavior, obj, position, rotation, sendTransform);
			
			return netBehavior;
		}
		[Obsolete("Use InstantiateDreamSpawner instead, its shorter and easier to type out ;)")]
		public DreamSpawnerBehavior InstantiateDreamSpawnerNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(DreamSpawnerNetworkObject[index]);
			var netBehavior = go.GetComponent<DreamSpawnerBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<DreamSpawnerBehavior>().networkObject = (DreamSpawnerNetworkObject)obj;
		...
		[Obsolete("Use InstantiateNightmareObject instead, its shorter and easier to type out ;)")]
		public NightmareObjectBehavior InstantiateNightmareObjectNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(NightmareObjectNetworkObject[index]);
			var netBehavior = go.GetComponent<NightmareObjectBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<NightmareObjectBehavior>().networkObject = (NightmareObjectNetworkObject)obj;
		...
		[Obsolete("Use InstantiateScoreTally instead, its shorter and easier to type out ;)")]
		public ScoreTallyBehavior InstantiateScoreTallyNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(ScoreTallyNetworkObject[index]);
			var netBehavior = go.GetComponent<ScoreTallyBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<ScoreTallyBehavior>().networkObject = (ScoreTallyNetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of DreamObjectBase
		/// </summary>
		/// <returns>
		/// A local instance of DreamObjectBaseBehavior
		/// </returns>
		/// <param name="index">The index of the DreamObjectBase prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public DreamObjectBaseBehavior InstantiateDreamObjectBase(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(DreamObjectBaseNetworkObject[index]);
			var netBehavior = go.GetComponent<DreamObjectBaseBehavior>();
		...
		/// <summary>
		/// Instantiate an instance of DreamSpawner
		/// </summary>
		/// <returns>
		/// A local instance of DreamSpawnerBehavior
		/// </returns>
		/// <param name="index">The index of the DreamSpawner prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public DreamSpawnerBehavior InstantiateDreamSpawner(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(DreamSpawnerNetworkObject[index]);
			var netBehavior = go.GetComponent<DreamSpawnerBehavior>();
		...
		/// <summary>
		/// Instantiate an instance of NightmareObject
		/// </summary>
		/// <returns>
		/// A local instance of NightmareObjectBehavior
		/// </returns>
		/// <param name="index">The index of the NightmareObject prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public NightmareObjectBehavior InstantiateNightmareObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(NightmareObjectNetworkObject[index]);
			var netBehavior = go.GetComponent<NightmareObjectBehavior>();
		...
		/// <summary>
		/// Instantiate an instance of ScoreTally
		/// </summary>
		/// <returns>
		/// A local instance of ScoreTallyBehavior
		/// </returns>
		/// <param name="index">The index of the ScoreTally prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public ScoreTallyBehavior InstantiateScoreTally(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(ScoreTallyNetworkObject[index]);
			var netBehavior = go.GetComponent<ScoreTallyBehavior>();
